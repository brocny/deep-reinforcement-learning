...
if __name__ == "__main__":
    ...
    while args.train:
        ...
        if ret_mean - ret_std > 150 and ret_mean - ret_std > ret_max:
            ret_max = ret_mean - ret_std
            network.actor_model.save(f'models-hc/actor-{time.strftime("%d-%m-%H:%M")}-r{int(round(ret_mean))}_{int(round(ret_std))}.h5')
            network.critic_model1.save(f'models-hc/critic1-{time.strftime("%d-%m-%H:%M")}-r{int(round(ret_mean))}_{int(round(ret_std))}.h5')
            network.critic_model2.save(f'models-hc/critic2-{time.strftime("%d-%m-%H:%M")}-r{int(round(ret_mean))}_{int(round(ret_std))}.h5')
            print('Saved trained model')
            if ret_mean - 2 * ret_std > 300:
                break
